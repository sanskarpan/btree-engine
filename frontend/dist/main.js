...
function axis(orient, scale) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain() : tickValues, format = tickFormat == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : identity_default : tickFormat, spacing = Math.max(tickSizeInner, 0) + tickPadding, range = scale.range(), range0 = +range[0] + offset, range1 = +range[range.length - 1] + offset, position = (scale.bandwidth ? center : number2)(scale.copy(), offset), selection = context.selection ? context.selection() : context, path = selection.selectAll(".domain").data([null]), tick = selection.selectAll(".tick").data(values, scale).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
...
// src/api/client.ts
var BASE = typeof window !== "undefined" ? `${window.location.origin}/api/v1` : "http://localhost:3001/api/v1";
async function get3(path2) {
  const res = await fetch(`${BASE}${path2}`);
  if (!res.ok)
    throw new Error(`GET ${path2}: ${res.status}`);
  return res.json();
}
async function post(path2, body) {
  ...
  if (!res.ok)
    throw new Error(`POST ${path2}: ${res.status}`);
  return res.json();
}
var engine = {
  stats: () => get3("/engine/stats"),
  crash: () => post("/engine/crash"),
  recover: () => post("/engine/recover")
};
var tree = {
  structure: () => get3("/tree/structure"),
  page: (id2) => get3(`/tree/page/${id2}`)
};
var mvcc = {
  versions: (key) => get3(`/mvcc/versions?key=${encodeURIComponent(key)}`),
  visibility: (key, txnId) => get3(`/mvcc/visibility?key=${encodeURIComponent(key)}&txnID=${txnId}`)
};
var buffer = {
  stats: () => get3("/buffer/stats"),
  frames: () => get3("/buffer/frames")
};
var wal = {
  tail: (n = 50) => get3(`/wal/tail?n=${n}`),
  checkpoint: () => post("/wal/checkpoint")
};
var scenarios = {
  list: () => get3("/scenarios"),
  run: (name) => post(`/scenarios/${name}/run`)
};
...
class WSClient {
  ...
  on(type2, handler) {
    const list = this.handlers.get(type2) ?? [];
    list.push(handler);
    this.handlers.set(type2, list);
    return () => {
      const updated = (this.handlers.get(type2) ?? []).filter((h) => h !== handler);
      this.handlers.set(type2, updated);
    };
  }
  onAll(handler) {
...
class BTreeVisualizer {
  svg;
  g;
  treeLayout = tree_default().nodeSize([NODE_W + 20, NODE_H + 60]);
  constructor(container) {
    this.svg = select_default2(container).append("svg").attr("width", WIDTH).attr("height", HEIGHT).attr("viewBox", `0 0 ${WIDTH} ${HEIGHT}`);
  ...
  async refresh() {
    try {
      const root2 = await tree.structure();
      this.render(root2);
    } catch {}
  }
  render(rootData) {
    const root2 = hierarchy(rootData, (d) => d.children ?? []);
    const treeData = this.treeLayout(root2);
    this.g.selectAll("*").remove();
    this.g.selectAll(".link").data(treeData.links()).join("path").attr("class", "link").attr("fill", "none").attr("stroke", "#999").attr("stroke-width", 1.5).attr("d", linkVertical().x((d) => d.x).y((d) => d.y));
...
// src/components/page/inspector.ts
var STATUS_COLOR = {
  committed: "#4caf50",
  active: "#ff9800",
  aborted: "#9e9e9e",
...
class PageInspector {
  ...
  async loadPage(id2) {
    try {
      const page = await tree.page(id2);
  ...
  renderPage(page) {
    ...
    const track = bar.append("div").style("width", "400px").style("height", "10px").style("background", "#eee").style("border-radius", "5px");
    track.append("div").style("width", `${fill}%`).style("height", "100%").style("background", fill > 80 ? "#e53935" : fill > 50 ? "#ff9800" : "#4caf50").style("border-radius", "5px");
    if (!page.slots?.length) {
      el.append("p").text("No slots.");
      return;
    }
    const table = el.append("table").style("border-collapse", "collapse").style("font-size", "12px");
    const thead = table.append("thead");
    const hrow = thead.append("tr");
    ["Slot", "Xmin", "Xmax", "Key", "Value", "Xmin Status", "Xmax Status"].forEach((h) => {
      hrow.append("th").text(h).style("border", "1px solid #ddd").style("padding", "4px 8px").style("background", "#f5f5f5");
    });
    ...
    page.slots.forEach((slot) => {
      ...
      [
        ...
        const td = tr.append("td").text(val).style("border", "1px solid #ddd").style("padding", "4px 8px");
        if (i === 5)
...
// src/components/versions/versions.ts
var COLORS = {
  live: "#4caf50",
  dead: "#e53935",
  active: "#ff9800",
...
class VersionChainViewer {
  ...
  buildUI() {
    ...
    const input = controls.append("input").attr("type", "text").attr("placeholder", "enter key...").style("width", "160px");
    controls.append("button").text("Show versions").style("margin-left", "6px").on("click", async () => {
      const key = input.node()?.value;
      if (key)
        await this.showVersions(key);
    });
    this.container.append("div").attr("id", "version-chart");
  }
  async showVersions(key) {
    try {
      const { versions } = await mvcc.versions(key);
  ...
  render(key, versions) {
    ...
    const xScale = linear2().domain([Math.min(...allXids) - 1, Math.max(...allXids) + 2]).range([60, 700]);
    const svg = el.append("svg").attr("width", 760).attr("height", versions.length * 40 + 60);
    const xAxis = axisBottom(xScale).ticks(8).tickFormat(format("d"));
    svg.append("g").attr("transform", `translate(0,${versions.length * 40 + 20})`).call(xAxis);
    svg.append("text").attr("x", 380).attr("y", versions.length * 40 + 55).attr("text-anchor", "middle").attr("font-size", "11px").text("Transaction ID");
    versions.forEach((v, i) => {
      const y2 = i * 40 + 30;
      const x1 = xScale(v.xmin);
      const x2 = v.xmax ? xScale(v.xmax) : 720;
      let color2 = COLORS.live;
      if (v.xmax && v.xmax_status === "committed")
        color2 = COLORS.dead;
      else if (v.xmin_status === "active")
        color2 = COLORS.active;
      else if (v.xmin_status === "aborted")
        color2 = COLORS.aborted;
      svg.append("rect").attr("x", x1).attr("y", y2 - 12).attr("width", Math.max(8, x2 - x1)).attr("height", 24).attr("rx", 4).attr("fill", color2).attr("opacity", 0.85);
      svg.append("text").attr("x", x1 + 4).attr("y", y2 + 4).attr("font-size", "10px").attr("fill", "white").text(`v=${v.value} xmin=${v.xmin} xmax=${v.xmax || "∞"}`);
    });
  }
}

// src/components/isolation/demo.ts
class IsolationDemo {
  container;
  constructor(el) {
    this.container = el;
  ...
  buildUI() {
    ...
    this.container.querySelector("#run-rc")?.addEventListener("click", async () => {
      const result = await scenarios.run("read-committed").catch((e) => ({ steps: [{ step: "error", description: String(e), ok: false }], name: "error", stats: {} }));
      this.renderSteps("#rc-steps", result.steps);
    });
    this.container.querySelector("#run-si")?.addEventListener("click", async () => {
      const result = await scenarios.run("write-skew").catch((e) => ({ steps: [{ step: "error", description: String(e), ok: false }], name: "error", stats: {} }));
      this.renderSteps("#si-steps", result.steps);
      const ws = result.stats?.["write_skew_occurred"];
      if (ws) {
  ...
  renderSteps(selector, steps) {
    const el = this.container.querySelector(selector);
    if (!el)
      return;
    el.innerHTML = steps.map((s) => `<div style="padding:4px;border-left:3px solid ${s.ok ? "#4caf50" : "#e53935"};margin:2px 0;background:${s.ok ? "#f1f8e9" : "#ffebee"}">
...
class BufferPoolPanel {
  ...
  constructor(container) {
    ...
    this.svg = select_default2(container).append("svg").attr("width", width).attr("height", 600);
    const ws = getWSClient();
  ...
  async refresh() {
    try {
      const [stats, framesResp] = await Promise.all([buffer.stats(), buffer.frames()]);
  ...
  render() {
    ...
    const frameGroups = this.svg.selectAll(".frame").data(this.frames).join("g").attr("class", "frame").attr("transform", (_d, i) => {
      const col = i % COLS;
      const row = Math.floor(i / COLS);
      return `translate(${10 + col * (FRAME_SIZE + 4)},${gridY + row * (FRAME_SIZE + 4)})`;
    });
    frameGroups.append("rect").attr("width", FRAME_SIZE).attr("height", FRAME_SIZE).attr("rx", 3).attr("fill", FRAME_COLOR).attr("stroke", "#ccc").attr("stroke-width", 1);
    frameGroups.append("text").attr("x", FRAME_SIZE / 2).attr("y", FRAME_SIZE / 2 + 4).attr("text-anchor", "middle").attr("font-size", "9px").attr("fill", "#333").text((d) => d.empty ? "" : `P${d.page_id}`);
...
class WALPanel {
  ...
  buildUI() {
    ...
    this.container.select("#btn-recover").on("click", async () => {
      ...
      await engine.recover().catch(() => {});
      status.text("Recovery complete.");
      this.refresh();
    });
    this.container.select("#btn-checkpoint").on("click", async () => {
      await wal.checkpoint().catch(() => {});
      this.refresh();
    });
  }
  async refresh() {
    try {
      const { records } = await wal.tail(100);
  ...
  renderLog() {
    ...
    this.records.forEach((r) => {
      const color2 = TYPE_COLOR[r.type] ?? "#333";
      el.append("div").style("padding", "2px 4px").style("border-left", `4px solid ${color2}`).style("margin", "1px 0").html(`<span style="color:${color2};font-weight:bold">${r.type}</span> LSN=${r.lsn} TxnID=${r.txn_id} PageID=${r.page_id} PrevLSN=${r.prev_lsn}`);
    });
  }
}
...
class EngineStore {
  ...
  handleEvent(evt) {
    ...
    switch (evt.Type) {
      case "txn_begin":
        s.activeTxns = new Set(s.activeTxns).add(evt.Extra.txn_id);
        break;
      case "txn_commit":
      case "txn_abort": {
        const next = new Set(s.activeTxns);
        next.delete(evt.Extra.txn_id);
        s.activeTxns = next;
        break;
      }
      case "tree_split":
        ...
        s.walRecords = [
          {
            lsn: evt.Extra.lsn,
            prev_lsn: 0,
            txn_id: evt.Extra.txn_id,
            page_id: 0,
            type: String(evt.Extra.type)
          },
          ...s.walRecords
        ].slice(0, 200);
        break;
  ...
  updateTree(root2) {
    this.state = { ...this.state, treeRoot: root2 };
    this.notify();
  }
...
var engineStore = new EngineStore;

// src/components/scenarios/panel.ts
class ScenariosPanel {
  ...
  async buildUI() {
    let scenarioList = [];
    try {
      const resp = await scenarios.list();
  ...
  renderResult(result) {
    const output = this.container.querySelector("#scenario-output");
    output.innerHTML = result.steps.map((s) => `<div style="padding:3px;border-left:3px solid ${s.ok ? "#4caf50" : "#e53935"};margin:2px 0;background:${s.ok ? "#f1f8e9" : "#ffebee"}">
            <b>${s.step}</b>: ${s.description}
          </div>`).join("");
    if (Object.keys(result.stats).length > 0) {
      output.innerHTML += `<div style="margin-top:8px;background:#f5f5f5;padding:6px;border-radius:4px"><b>Stats:</b><pre style="margin:0;font-size:11px">${JSON.stringify(result.stats, null, 2)}</pre></div>`;
    }
  }
  startMetrics() {
    const update = async () => {
      try {
        const [stats, bufStats] = await Promise.all([engine.stats(), buffer.stats()]);
  ...
  buildEventChart() {
    const chartEl = this.container.querySelector("#metrics-charts");
    if (!chartEl)
      return;
    const counts = {};
    const ws = getWSClient();
    ...
    const svg = select_default2(chartEl).append("svg").attr("width", 700).attr("height", 120);
    ...
    function redraw() {
      ...
      const barH = 12;
      const w = 600;
      g.selectAll(".bar-g").data(entries, ([k]) => k).join((enter) => {
        ...
        return bg;
      }, (update) => update.attr("transform", (_d, i) => `translate(0,${i * (barH + 4)})`)).each(function([k, v]) {
        const barW = v / max * w;
        select_default2(this).select("rect").attr("width", barW);
        select_default2(this).select("text").text(`${k}: ${v}`);
        select_default2(this).select(".label").text(k);
      });
    }
  }
}
