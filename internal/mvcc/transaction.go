...
type UndoRecord struct {
	...
	OldXmax uint64     // previous delete marker restored by UndoDelete
...
type TransactionManager struct {
	...
	recoveredCommittedFloor atomic.Uint64
...
func (tm *TransactionManager) GetStatus(id TxnID) TxnStatus {
	...
	if id != 0 {
		if floor := TxnID(tm.recoveredCommittedFloor.Load()); floor != 0 && id < floor {
			return TxnCommitted
		}
	}
...
// SetRecoveredCommittedFloor marks all unknown txn IDs below nextTxnID as
// committed after crash recovery. Explicit statuses in statusTable still win.
func (tm *TransactionManager) SetRecoveredCommittedFloor(nextTxnID TxnID) {
	tm.recoveredCommittedFloor.Store(uint64(nextTxnID))
}
