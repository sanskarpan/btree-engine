...
func (w *WALManager) WriteCheckpoint(dpt map[uint32]uint64, att map[uint64]ATTEntry) (uint64, error) {
	cr := &CheckpointRecord{DPT: dpt, ATT: att}
...
func EncodeCLRPayload(undoNextLSN uint64, undonePayload []byte) []byte {
	buf := make([]byte, 8+len(undonePayload))
	binary.LittleEndian.PutUint64(buf[0:], undoNextLSN)
	copy(buf[8:], undonePayload)
	return buf
}
